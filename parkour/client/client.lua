...
function GetEntInFrontOfPlayer(Ped)
    ...
    local firstHit = nil
    local lastHit = nil
    while heightIndex <= HeightLevels.Max do
    ...
    return firstHit, lastHit, endCoords
...
function reverseVault(playerPed, heightLevel)
    ...
    print('difference: ' .. tostring(HeightLevels.High - heightLevel))
    if HeightLevels.High - heightLevel <= 0.5 then
        Wait(1200)
    else
        Wait(700)
    end
...
function ledgeJumpUp(playerPed)
    local x, y, z = table.unpack(GetEntityCoords(playerPed))

    local animTime = doAnimation(playerPed, ParkourAnimations.ledge.ledgeJumpUp)
    Wait(animTime * 1000)

    SetEntityCoords(playerPed, x, y, z + 1.43, true, false, false, false)
    SetEntityHasGravity(playerPed, false)
...
RegisterCommand('+parkour', function()
    ...
    local firstHit, lastHit, endCoords = GetEntInFrontOfPlayer(playerPed)

    print('firstHit: ' .. tostring(firstHit))
    print('lastHit: ' .. tostring(lastHit))

    if firstHit ~= nil then
        if firstHit > HeightLevels.Min and firstHit <= HeightLevels.Low then
            slide(playerPed)
        elseif lastHit <= HeightLevels.Medium then
            kashVault(playerPed)
        elseif lastHit <= HeightLevels.High then
            reverseVault(playerPed, lastHit)
        elseif lastHit < HeightLevels.Max then
            ledgeJumpUp(playerPed)
        else
            wallFlip(playerPed)
        end
    end
